...
namespace Web.Controllers.ModelSecurity
{
    ...
    public class PermissionController : ControllerBase
    {
        ...
        [ProducesResponseType(404)]
        [ProducesResponseType(500)]
        public async Task<IActionResult> DeletePermission(DeleteRequest deleteRequest)
        {
            try
            {
               await _PermissionBusiness.DeleteAsyncStrategy(deleteRequest.Id, deleteRequest.Strategy);
                return Ok(); // Código 204: Eliminación exitosa sin contenido
            }
            catch (ValidationException ex)
            {
                _logger.LogWarning(ex, "Validación fallida al eliminar el permission con ID: {PermissionId}");
                return BadRequest(new { message = ex.Message });
            }
            catch (EntityNotFoundException ex)
            {
                _logger.LogInformation(ex, "Permission no encontrado con ID: {PermissionId}");
                return NotFound(new { message = ex.Message });
            }
            catch (ExternalServiceException ex)
            {
                _logger.LogError(ex, "Error al eliminar el permission con ID: {PermissionId}");
                return StatusCode(500, new { message = ex.Message });
            }
        }
    }
